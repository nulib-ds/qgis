...
If you're not familiar with Python, this is a great way for you to try it out. We'll only be copying and pasting the code below, but remember that Python can be a powerful tool you can use to automate geoprocessess. 
...
#### You can also bring in basemaps by using the QuickOSM PlugIn. QGIS Plugins are similar to a library and add additional functionality to the QGIS application. These plugins can be installed directly from the QGIS Plugin Manager within the QGIS application. 
