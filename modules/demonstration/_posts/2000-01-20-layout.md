...
**There are many ways to add in basemaps, or scalable, referenced image layers. The example below uses Python.** 
...
**You can also bring in basemaps by using the QuickOSM PlugIn. [QGIS Plugins](https://plugins.qgis.org/) are similar to a library and add additional functionality to the QGIS application. These plugins can be installed directly from the QGIS Plugin Manager within the QGIS application.** 
